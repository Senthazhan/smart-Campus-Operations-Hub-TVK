...
export function NotificationsPage() {
  ...
  async function refresh() {
    ...
    setError(null);
    try {
      const d = await listNotifications({ page, size: 10 });
      setData(d);
    } catch (e) {
  ...
  const getNoticeConfig = (type) => {
    switch (type) {
      case 'BOOKING_APPROVED': case 'TICKET_RESOLVED':
        return { icon: CheckCircle2, color: 'text-success', bg: 'bg-success/5', border: 'border-success/20' };
      case 'BOOKING_REJECTED': case 'TICKET_REJECTED':
    ...
    <div className="max-w-5xl mx-auto space-y-10 animate-fade-in-up pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        <div className="space-y-2">
           ...
           <p className="text-[var(--color-muted)] font-medium max-w-lg">Manage real-time operational signals and deployment updates across the campus ecosystem.</p>
        </div>
      ...
      <div className="flex items-center gap-4 px-2 pb-2 overflow-x-auto no-scrollbar">
         {['ALL', 'UNREAD', 'CRITICAL', 'DEPLOYMENTS'].map((f, i) => (
           <button key={f} className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border ${i === 0 ? 'bg-primary border-primary text-white shadow-premium' : 'bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-muted)] hover:border-slate-400'}`}>
              {f}
           </button>
         ))}
      ...
      <div className="grid gap-6">
        {loading && !data ? (
            ...
            <div className="space-y-4">
              {data.content.map((n) => {
                ...
                return (
                  <Card 
                    key={n.id} 
                    className={`group transition-all duration-500 overflow-hidden border-[var(--color-border)] rounded-[2rem] shadow-soft hover:shadow-premium hover:-translate-y-1 ${!n.read ? 'bg-[var(--color-surface)] border-primary/20' : 'bg-[var(--color-surface)] opacity-80'}`}
                  >
                    {!n.read && <div className="absolute top-0 right-0 p-4"><div className="w-2 h-2 rounded-full bg-primary animate-pulse" /></div>}
                         ...
                         <div className="flex items-start justify-between gap-6">
                            <div className="space-y-1">
                               <div className="flex items-center gap-3">
                                  ...
                                  {!n.read && <Badge className="bg-primary/10 text-primary border-0 text-[8px] font-black uppercase tracking-widest px-2">New Signal</Badge>}
                               </div>
                               ...
                               {!n.read && (
                                 <Button
                                   ...
                                   onClick={async (e) => {
                                     e.stopPropagation();
                                     setBusyId(n.id);
                                     try { await markNotificationRead(n.id); await refresh(); } finally { setBusyId(null); }
                                   }}
                                   className="h-10 px-6 text-[9px] font-black uppercase tracking-widest border-[var(--color-border)] bg-[var(--color-background)] hover:border-primary transition-all shadow-sm shrink-0"
                                 >
          ...
          <Card className="py-32 text-center border-dashed border-[var(--color-border)] bg-[var(--color-background)]/50 rounded-[3rem] animate-fade-in-up">
             ...
             <p className="text-[var(--color-muted)] max-w-xs mx-auto mt-3 text-xs font-bold uppercase tracking-widest opacity-60 leading-relaxed">No active alerts detected in the current stream buffer. Dispatching heartbeat sync...</p>
             <Button 
               variant="secondary" 
      ...
      <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-10 rounded-[2.5rem] shadow-soft flex flex-col md:flex-row items-center gap-10">
         ...
         <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-[var(--color-muted)] text-[12px] font-bold mt-2 leading-relaxed uppercase tracking-wider">Signals are prioritized based on operational impact. Critical alerts (Red) require immediate intervention, while system updates (Blue) are provided for context only.</p>
         </div>
      </div>
    </div>
  );
